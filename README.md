...
A client library to connect to Rhombus Core RPC in JavaScript.

## Get Started

rhombus-bitcoind-rpc runs on [node](http://nodejs.org/), and can be installed via [npm](https://npmjs.org/):

```bash
npm install rhombus-bitcoind-rpc
```
...
var run = function() {
  ...
  var RpcClient = require('rhombus-bitcoind-rpc');
